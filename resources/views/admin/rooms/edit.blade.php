...
@section('content')
    <form action="{{ route('admin.rooms.update', $room->id) }}" method="post">
        ...
        <div class="mb-3">
            ...
            <input required type="text" class="form-control form-control-sm" id="name" name="name" value="{{ $room->name }}">
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input  type="text" value="{{ $room->comment }}" class="form-control form-control-sm" id="comment" name="comment">
            @error('comment')
            <p class="text-danger">{{ __($message) }}</p>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input  required type="text" value="{{ $room->slug }}" class="form-control form-control-sm" id="slug" name="slug">
            @error('slug')
            <p class="text-danger">{{ __($message) }}</p>
            @enderror
        </div>
