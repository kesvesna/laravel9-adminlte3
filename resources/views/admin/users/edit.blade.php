...
@section('content')
    <form action="{{ route('admin.users.update', $user->id) }}" method="post">
        ...
        <div class="form-group pt-3">
            ...
            <input class="form-control" required name="id" id="id" type="text" value="{{$user->id}}" readonly>
        </div>
        <div class="form-group pt-3">
            <label for="name">Фамилия И.О.</label>
            <input class="form-control" required name="name" id="name" type="text" value="{{$user->name}}">
        </div>
        <div class="form-group">
            <label for="email">Электронная почта</label>
            <input class="form-control" required name="email" id="email" type="email" value="{{$user->email}}">
        </div>
        <div class="form-group">
            <label for="password">Пароль</label>
            <input class="form-control" required name="password" id="password" type="text" value="{{$user->password}}">
        </div>
        <div class="form-group">
            <label for="user_status_id">Статус</label>
            <select required class="form-control" name="user_status_id" aria-label="select">
        ...
        <div class="form-group">
            ...
            <select required class="form-control" name="visible" aria-label="select">
