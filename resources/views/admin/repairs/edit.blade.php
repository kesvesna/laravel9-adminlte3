...
@section('content')
    <form action="{{ route('admin.repairs.update', $repair->id) }}" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control form-control-sm" id="id" value="{{ $repair->id }}" readonly>
        </div>
        <div class="mb-3">
            <label class="mb-2" for="plan_date">Запланирован на</label>
            <br>
            <input value="{{ $repair->currentHistory->plan_date }}" type="datetime-local" id="plan_date" name="plan_date" class="form-control form-control-sm" style="background: rgba( 255, 255, 255, 0.5 );">
        </div>
        @if($repair->currentHistory->application_id > 0)
            <div class="mb-3">
                <label for="id" class="form-label">ID заявки</label>
                <input type="text" class="form-control form-control-sm" id="application_id" name="application_id" value="{{ $repair->application_id }}" readonly>
            </div>
        @endif
