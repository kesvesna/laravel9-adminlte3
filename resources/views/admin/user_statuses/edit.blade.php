...
@section('content')
    <form action="{{ route('admin.user_statuses.update', $user_status->id) }}" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control form-control-sm" id="id" value="{{ $user_status->id }}" readonly>
        </div>
        <div class="mb-3">
            <label for="comment" class="form-label">Название</label>
            <input type="text" class="form-control form-control-sm" name="name" id="name" value="{{ $user_status->name }}">
        </div>
        <div class="mb-3">
            <label for="comment" class="form-label">Slug</label>
            <input type="text" class="form-control form-control-sm" name="slug" id="slug" value="{{ $user_status->slug }}">
        </div>
        <div class="mb-3">
            <label for="sort_order" class="form-label">Номер в сортировке</label>
            <input type="number" class="form-control form-control-sm" name="sort_order" id="sort_order" value="{{ $user_status->sort_order }}">
        </div>
